...
import '../common/skeleton_row.dart';

/// Inline subscriber list for a status page, rendered on the show view.
///
/// Reads from [StatusPageSubscriberController] directly so the status-page
/// detail screen can surface the audience without a dedicated route. Remove
/// is optimistic and protected by a confirm dialog, matching the old
/// standalone view behavior.
class SubscribersSection extends StatefulWidget {
  const SubscribersSection({super.key, required this.statusPageId});
...
class _SubscribersSectionState extends State<SubscribersSection> {
  StatusPageSubscriberController get _c =>
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      _c.load(widget.statusPageId);
    });
  }

  Future<void> _refresh() => _c.load(widget.statusPageId);

  @override
  Widget build(BuildContext context) {
    return FormSectionCard(
      titleKey: 'subscriber.list.title',
      subtitleKey: 'subscriber.list.subtitle',
      icon: Icons.mark_email_read_outlined,
      child: AnimatedBuilder(
        animation: _c,
        builder: (_, _) => _c.renderState(
          (subscribers) => subscribers.isEmpty
              ? _empty()
              : WDiv(
                  className: 'flex flex-col gap-2',
                  children: [
                    for (final subscriber in subscribers) _row(subscriber),
                  ],
                ),
  ...
  Future<void> _remove(StatusPageSubscriber subscriber) async {
    ...
    if (!ok) return;
    final removed = await _c.remove(subscriber.id);
